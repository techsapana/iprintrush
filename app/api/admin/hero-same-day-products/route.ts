...
import { SAME_DAY_PRINTING_CATEGORY_SLUG } from '@/app/lib/siteConstants';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

async function assertSameDayEligibleProductIds(ids: string[]): Promise<string[]> {
  ...
  const rows: any[] = (await query(
    `SELECT p.id FROM products p
     ...
     WHERE p.id IN (${placeholders})
       ...
       AND (
         c.slug = ?
         OR IFNULL(p.same_day_eligible, 0) = 1
       )`,
    [...ids, SAME_DAY_PRINTING_CATEGORY_SLUG],
  )) as any[];
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rows: any[] = (await query(
      'SELECT product_id FROM hero_same_day_products ORDER BY sort_order ASC, id ASC',
    )) as any[];
    return NextResponse.json({ productIds: rows.map((r) => String(r.product_id)) });
  } catch (error: any) {
    console.error('admin hero-same-day GET:', error);
  ...
  try {
    if (!(await hasDbTable('hero_same_day_products'))) {
      return NextResponse.json(
        { error: 'Table hero_same_day_products does not exist. Run the SQL migration first.' },
        { status: 503 },
      );
    }
    ...
    for (const productId of validated) {
      await query(
        'INSERT INTO hero_same_day_products (product_id, sort_order) VALUES (?, ?)',
        [productId, order],
      );
      order += 1;
    }
    ...
    console.error('admin hero-same-day PUT:', error);
