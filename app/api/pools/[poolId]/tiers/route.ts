...
export async function POST(req: NextRequest, { params }: { params: Promise<{ poolId: string }> }) {
  try {
    ...
    const { label, minQty, maxQty, unitPrice, discountPercent = 0, displayOrder, enabled } = body;
    const discount = Number(discountPercent);

    // Insert new tier
    const result = await queryOne(
      `INSERT INTO customization_quantity_tiers (pool_id, label, min_qty, max_qty, unit_price, discount_percent, display_order, enabled) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        poolId,
        label,
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ poolId: string }> }) {
  try {
    const { poolId } = await params;
    const body = await req.json();
    const { tierId, label, minQty, maxQty, unitPrice, discountPercent, displayOrder, enabled } =
      body;
    const discount = Number(discountPercent);
    ...
    await queryOne(
      `UPDATE customization_quantity_tiers 
       SET label = ?, min_qty = ?, max_qty = ?, unit_price = ?, discount_percent = ?, display_order = ?, enabled = ? 
