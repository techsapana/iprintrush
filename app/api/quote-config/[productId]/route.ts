...
async function getFullConfig() {
  ...
  return {
    decorations: (decorations as any[]).map((d: any) => ({
      id: d.id,
      name: d.name,
      priceModifier: Number.isFinite(parseFloat(d.price_modifier)) ? parseFloat(d.price_modifier) : 0,
    ...
    sizes: (sizes as any[]).map((s: any) => ({
      id: s.id,
      label: s.label,
      priceAddon: Number.isFinite(parseFloat(s.price_addon)) ? parseFloat(s.price_addon) : 0,
...
quantityTiers: (tiers as any[]).map((t: any) => ({
    ...
    printLocations: (printLocations as any[]).map((p: any) => ({
      id: p.id,
      name: p.name,
      priceModifier: Number.isFinite(parseFloat(p.price_modifier)) ? parseFloat(p.price_modifier) : 0,
      enabled: Boolean(p.enabled),
    })),
    turnarounds: (turnarounds as any[]).map((t: any) => ({
      id: t.id,
      name: t.name,
      priceModifier: Number.isFinite(parseFloat(t.price_modifier)) ? parseFloat(t.price_modifier) : 0,
      enabled: Boolean(t.enabled),
      pricingType: t.pricing_type || 'flat',
      percentageValue: t.percentage_value != null ? (Number.isFinite(parseFloat(t.percentage_value)) ? parseFloat(t.percentage_value) : 0) : 0,
    })),
    ...
    shipping: {
      ...
      oversizedWidthThresholdIn: Number.isFinite(parseFloat(shippingConfig?.oversized_width_threshold_in || 0)) ? parseFloat(shippingConfig?.oversized_width_threshold_in || 0) : 0,
      oversizedWeightThresholdLb: Number.isFinite(parseFloat(shippingConfig?.oversized_weight_threshold_lb || 0)) ? parseFloat(shippingConfig?.oversized_weight_threshold_lb || 0) : 0,
      ...
      rules: (shippingRules as any[]).map((r: any) => ({
        ...
        price: Number.isFinite(parseFloat(r.price)) ? parseFloat(r.price) : 0,
