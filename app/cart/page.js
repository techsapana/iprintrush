                ...
                {items.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      {item.options?.customizationsDisplay && Object.keys(item.options.customizationsDisplay).length > 0 && (
                        <div className="mt-2 text-xs text-gray-700 space-y-0.5">
                          {Object.entries(item.options.customizationsDisplay)
                            .filter(([k]) => !/size\s*breakdown/i.test(String(k)))
                            .map(([k, v]) =>
                            v ? (
                              <div key={k}>
                      ...
                      {item.options?.deliveryMethod && (
                        <p className="text-sm text-gray-600 mt-1">
                          Delivery: {item.options.deliveryMethod === 'shipping' ? 'Shipping' : 'Pickup'}
                        </p>
                      )}
                      ...
                      <div className="flex items-center gap-2 mt-4">
                        <button
                          onClick={() =>
                            updateQuantity(item.id, Math.max(1, item.quantity - 1), item.options)
                          }
